...
import {
  Activity, CalendarDays, Download, Edit3, Flame, Gauge, Trash2, Trophy, Zap,
} from "lucide-react";
import { useApp } from "../context/AppContext";
import { computeStreak, dayDiff, formatDate, toLocalDateString } from "../lib/tracker";
...
export function Dashboard({ onNavigate }: { onNavigate: (p: Page) => void }) {
  ...
  const exportFile = (type: "csv" | "json") => {
    ...
    if (type === "json") {
      content = JSON.stringify({ startDate, baselineWpm, baselineAccuracy, days: rows }, null, 2);
      mime = "application/json"; ext = "json";
    } else {
      const header = "day,date,wpm,accuracy";
      const body = rows.map((r) => `${r.day},${r.date},${r.wpm},${r.accuracy}`).join("\n");
      content = `${header}\n${body}`; mime = "text/csv;charset=utf-8"; ext = "csv";
    }
    ...
    <div className="mx-auto max-w-[1200px] px-4 py-16 sm:px-6 lg:px-8 lg:py-20 kf-page-in">
      ...
      <section className="mb-16 max-w-3xl">
        <div className="mb-4 flex items-center gap-2 text-xs uppercase tracking-[0.18em] text-[var(--kf-muted)]">
          <span className="kf-pulse inline-block h-1.5 w-1.5 rounded-full bg-[var(--kf-primary)]" />
          {t.tagline}
        </div>
      ...
      <section className="mb-16 grid grid-cols-2 gap-px overflow-hidden rounded-lg border border-[var(--kf-hairline)] bg-[var(--kf-hairline)] sm:grid-cols-3 lg:grid-cols-6 kf-stagger">
        <Stat icon={<Flame size={15} />} label={t.streak} value={`${stats.streak}`} unit={t.days} />
        <Stat icon={<Zap size={15} />} label={t.current} value={`${stats.current}`} unit="WPM" />
        <Stat icon={<Trophy size={15} />} label={t.max} value={`${stats.maxWpm}`} unit="WPM" />
        <Stat icon={<Activity size={15} />} label={t.maxAccuracy} value={`${stats.maxAcc.toFixed(1)}`} unit="%" />
        <Stat icon={<Gauge size={15} />} label={t.avg} value={`${stats.avgAcc.toFixed(1)}`} unit="%" />
        <Stat
      ...
      <section className="rounded-lg border border-[var(--kf-hairline)] bg-[var(--kf-canvas)]">
        ...
        {chartData.length === 0 ? (
          <div className="flex min-h-40 flex-col items-center justify-center gap-3 p-8 text-[var(--kf-muted)]">
            <CalendarDays size={22} />
            <p className="text-sm">{t.noDays}</p>
            <button
              onClick={() => onNavigate("new")}
              className="mt-1 inline-flex h-9 items-center gap-2 rounded-md bg-[var(--kf-primary)] px-4 text-xs font-medium text-[var(--kf-on-primary)] transition-colors duration-150 hover:bg-[var(--kf-primary-active)]"
            >
              {t.add}
            </button>
          </div>
              ...
              <tbody>
                {chartData.map((entry) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-3.5 text-[var(--kf-body)]">{formatDate(entry.date, lang)}</td>
