  ...
  return (
    <div className="mx-auto max-w-[860px] px-4 py-16 sm:px-6 lg:px-8 lg:py-24 kf-page-in">
      ...
      <section className="rounded-lg border border-(--kf-hairline) bg-(--kf-canvas)">
        <ul className="divide-y divide-(--kf-hairline-soft)">
          {[t.helpBullet1, t.helpBullet2, t.helpBullet3, t.helpBullet4].map(
            (line, i) => (
              <li key={i} className="flex gap-4 p-6">
                <span className="mt-0.5 inline-flex h-6 w-6 shrink-0 items-center justify-center rounded-md bg-(--kf-surface-card) text-xs font-medium text-(--kf-ink)">
                  {i + 1}
                </span>
                <p className="text-sm leading-relaxed text-(--kf-body)">
                  {line}
                </p>
              </li>
            ),
